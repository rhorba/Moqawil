...
|---|---|---|---|---|
| S12-01 | Draft Terms of Service (FR primary + EN) — hosted-instance-specific: AGPL notice distinguishing self-host vs. hosted, service description, liability limits, account termination, AE data-processing terms | PM + Legal-adjacent drafting | M | USER (legal review) | **DONE** → `docs/terms-of-service-moqawil.md` |
| S12-02 | Draft Privacy Policy (FR + EN) — CNDP-oriented: what's collected (ICE/IF/invoices/client PII per the data model in root `CLAUDE.md` §8), 10-year retention (CGI Art. 211), rights of access/rectification/erasure, no third-party sharing beyond the BAM rate scrape (which touches no personal data), breach-notification posture | PM + Legal-adjacent drafting | M | USER (legal review) | **DONE** → `docs/privacy-policy-moqawil.md` |
| S12-03 | CNDP data-controller registration checklist — what the filing requires (legal entity identity, processing purpose, security-measures summary, data-retention schedule), assembled as a document the owner can hand to counsel or file directly | Security Engineer | S | USER (files it) | **DONE** → `docs/cndp-registration-checklist-moqawil.md` |
...
| ID | Task | Specialist | Size | Handoff-To |
|---|---|---|---|---|
| S12-04 | Write a pentest/security-audit scope document for an external vendor: attack surface summary, what's already covered by CI (Semgrep SAST, Trivy SCA, Gitleaks) vs. what needs a human (auth flows, cross-tenant IDOR under the hosted multi-tenant model, rate-limiter bypass, session handling), links to the two prior IDOR audits (`.logs/issues.md`, Sprint 9 + Sprint 11) | Security Engineer | M | USER (commissions the vendor) | **DONE** → `docs/pentest-scope-moqawil.md` |
| S12-05 | One-page security-posture summary suitable for handing to an auditor or a security-conscious prospective hosted customer | Security Engineer | S | USER | **DONE** → `docs/security-posture-summary-moqawil.md` |
...
| ID | Task | Specialist | Size | Handoff-To |
|---|---|---|---|---|
| S12-07 | 3 bilingual (FR primary + AR) SEO blog posts for the `moqawil/docs/` Docusaurus site: "Comment déclarer son CA d'auto-entrepreneur en 2026", "Le plafond de 80 000 DH par client expliqué", "Auto-entrepreneur Maroc : éviter la perte du statut" | Copywriter + Frontend Dev (docs site) | M | Tester (link/build check) | **DONE** → `moqawil/docs/docs/article-*.md` + `moqawil/docs/i18n/ar/...` — build-verified both locales |
| S12-08 | GitHub public-release prep: CHANGELOG, verify README version/feature-list is current (Sprint 10 already refreshed once — recheck after Sprint 11's SaaS-readiness additions), confirm `LICENSE` (AGPL-3.0 root) and `packages/tax-engine/LICENSE` (Apache-2.0) are both present and correctly referenced | DevOps/DevSecOps | S | Project Monitor | **DONE** → `moqawil/CHANGELOG.md` (new); root `LICENSE` was stale MIT, corrected to AGPL-3.0; `packages/tax-engine/LICENSE` confirmed Apache-2.0; README test badge refreshed to the Batch 5 verification run's real count |
| S12-09 | Draft announcement posts (content only, not submission): r/Maroc, r/MoroccanDevs, Facebook group "Auto-entrepreneurs Maroc" (~90K members), LinkedIn post targeting Moroccan chartered accountants, Show HN | Copywriter | M | USER (see caveat) | **DONE** → `docs/announcement-drafts-moqawil.md` — none submitted |
| S12-10 | Outreach list: 3-5 named chartered accountants to approach for endorsements post-launch (root `CLAUDE.md` §16, "Weeks 2-4") | PM | S | USER | **DONE** → `docs/accountant-outreach-list-moqawil.md` — sourcing framework + template, not fabricated names (see doc for why) |
...
| ID | Task | Specialist | Size | Handoff-To |
|---|---|---|---|---|
| S12-11 | typecheck + lint + full Vitest + full Playwright green (re-run per the Sprint 11 ICE-fixture-fix baseline: `pnpm test` all 14 files, `playwright test --workers=1`) | Tester | S | Project Monitor | **DONE** — typecheck/lint clean, Vitest 192 passing/7 skipped, coverage 100/100/100/92.78% (gate clears), Playwright 21/21 passing/3 skipped against a real `next start` build (CI-matching) after an initial `next dev` run hit the project's known cold-compile flakiness class, not a regression |
| S12-12 | Sprint snapshot → `.logs/metrics.md`; `git push origin master` (Framework Rule 3) | Project Monitor | S | USER | **DONE** — snapshot logged; push follows |

---

## Definition of Done (Sprint 12 closes)
- [x] ToS + Privacy Policy drafts exist, explicitly marked "pending lawyer review" — not silently treated as final
- [x] CNDP registration checklist exists
- [x] Pentest scope doc + security-posture summary exist and are handed to the owner
- [x] Deployment runbook exists and covers VPS/DNS/HTTPS/backups/monitoring end to end
- [x] 3 bilingual blog posts published to the docs site
- [x] CHANGELOG + README + LICENSE files verified current
- [x] Announcement post drafts exist for all 5 named channels, none submitted without explicit per-post user confirmation
- [x] Accountant outreach list exists (as a sourcing framework, not fabricated names — see S12-10 doc)
- [x] 0 regressions — full test suite + e2e stay green
